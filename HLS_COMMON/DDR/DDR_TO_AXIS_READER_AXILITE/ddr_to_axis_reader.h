 ...
 * 				andr� a leggere il frame buffer a frame_index - 1
 * @frame_count indica il numero totale di frame letti da questo modulo dal momento in cui
 * 				� stato inizializzato (da usare in fase di debug)
 * @frame_count_equals  indica il numero di volte in cui questo modulo � andato a leggere
...
void ddr_to_axis_reader(hls::stream<DATA_OUT> &outStream, volatile u64* base_ddr_addr, ap_uint<FRAME_INDEX_WIDTH>* frame_index,
						u32 frame_buffer_dim, u32 frame_buffer_offset, u8 frame_buffer_number, bool update_intr);
