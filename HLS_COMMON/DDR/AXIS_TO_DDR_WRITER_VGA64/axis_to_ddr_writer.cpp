...
{
	/*
	 * La depth � usata solo in fase di test (co-simulazione). Per FRAME_BUFFER_DIM = 128
	 * porre depth = 32
	 *
	 ...
	 * ma il buffer � sempre unico. Per potere fare il cast da buffer a buffer_pixel
	 * devo usare dei dati noti, non me lo fa fare con ap_uint<>
	 *
	 * Probabilmente il fatto che il buffer utilizzi 2 BRAM � dovuto non tanto
	 * alla dimensione del buffer, che � comunque pi� piccola di quella di una
	...
	*frame_index = inner_index;
	...
	//L'offset � usato con il puntatore a long, motivo per cui devo sempre dividere per 8
	int offset = inner_index*FRAME_OFFSET/sizeof(u64);

	/*
	 * Il ciclo pi� esterno indica quante volte faccio un trasferimento
	 * dal modulo alla DDR, cio� a FRAME_BUFFER_DIM / BUFFER_SIZE volte.
	...
	for(int idx = 0; idx < FRAME_BUFFER_DIM / BUFFER_SIZE; idx++)
	{
		...
		for(int j = 0; j < BUFFER_SIZE / sizeof(u64); j++)
		{
			...
			/*
			 * Il ciclo pi� interno si occupa di riempire il registro a 8 byte temp
			 * prendendo in ingresso dati di lunghezza possibilmente pi� piccola
			 */
			for(int i = 0; i < sizeof(u64) / sizeof(DATA_IN); i++)
			{
				...
				if (sizeof(DATA_IN) != 8)
				{
					data = data << 64 - sizeof(DATA_IN)*8; 	//Porto il dato letto nei bit pi� significativi
	 ...
	 * il numero di frame scritti realmente sar� frame count - 1
	 */
	frame_count_inner++;
