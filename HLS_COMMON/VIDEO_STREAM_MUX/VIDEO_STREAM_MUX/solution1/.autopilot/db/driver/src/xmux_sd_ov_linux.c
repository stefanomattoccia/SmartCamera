...
typedef struct {
    ...
    XMux_sd_ov_uio_map maps[ MAX_UIO_MAPS ];
} XMux_sd_ov_uio_info;

/***************** Variable Definitions **************************************/
static XMux_sd_ov_uio_info uio_info;
...
static int uio_info_read_name(XMux_sd_ov_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMux_sd_ov_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMux_sd_ov_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMux_sd_ov_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMux_sd_ov_Initialize(XMux_sd_ov *InstancePtr, const char* InstanceName) {
	XMux_sd_ov_uio_info *InfoPtr = &uio_info;
