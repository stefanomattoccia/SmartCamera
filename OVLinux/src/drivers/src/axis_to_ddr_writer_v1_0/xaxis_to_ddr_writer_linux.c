...
typedef struct {
    ...
    XAxis_to_ddr_writer_uio_map maps[ MAX_UIO_MAPS ];
} XAxis_to_ddr_writer_uio_info;

/***************** Variable Definitions **************************************/
static XAxis_to_ddr_writer_uio_info uio_info;
...
static int uio_info_read_name(XAxis_to_ddr_writer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAxis_to_ddr_writer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAxis_to_ddr_writer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAxis_to_ddr_writer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAxis_to_ddr_writer_Initialize(XAxis_to_ddr_writer *InstancePtr, const char* InstanceName) {
	XAxis_to_ddr_writer_uio_info *InfoPtr = &uio_info;
