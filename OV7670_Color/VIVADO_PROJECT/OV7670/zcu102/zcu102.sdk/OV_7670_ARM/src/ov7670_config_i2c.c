...
#define FIRMWARE_LENGTH 50
	...
	firmware[32].Register_Data 		= 0x02;
	...
	firmware[35].Register_Data 		= 0xB5;	// 0xB5 enables pattern bar
	firmware[35].last_one			= 1;
...
int OV7670_set_VGA_resolution_and_YUV_output()
{
	...
	xil_printf("\n--------SETTING VGA RESOLUTION and YUV OUTPUT--------\n");
	...
	xil_printf("--------end SETTING VGA RESOLUTION and YUV OUTPUT--------\n");
...
int OV7670_prescale_clock_and_PLL_control(int input_clock_frequency, int internal_clock_frequency, int PLL_value_int)
{
	...
	//Se l'input clock coincide con l'internal clock non è necessario applicare alcun pre-scale
	if(input_clock_frequency == internal_clock_frequency)
	{
		xil_printf("\nNon è necessario applicare il clock pre-scalar: input_clock_frequency == internal_clock_frequency");
		return XST_FAILURE;
	}
